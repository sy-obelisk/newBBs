...
class uc_note {
    ...
    function __construct(){
        ...
        $this->dbLink->connect('rm-2zey73j5g3cwl2sji.mysql.rds.aliyuncs.com', 'syzxleigewang', '4q8YmF48E78y', 'db_liuxue', 0, true, 'utf-8');
    }
    ...
    function synlogin($get, $post) {
        ...
        if(!$u){
            ...
            $sql = "select * from x2_user WHERE  $where";
            $u = $this->dbLink->fetch_first($sql);
            if (!$u) {
                $time = time();
                $sql = "INSERT INTO x2_user (`userName`,`email`,`userPass`,`phone`,`createTime`,`uid`) VALUES ('{$username}','{$email}','{$password}','{$phone}','{$time}','{$uid}')";
                $this->dbLink->query($sql);
                ...
                $userId = $u['id'];
                if($phone != $u['phone']){
                    $sql = "UPDATE x2_user SET phone = '$phone' WHERE id = {$u['id']}";
                    $this->dbLink->query($sql);
                }
                if($email != $u['email']){
                    $sql = "UPDATE x2_user SET email = '$email' WHERE id = {$u['id']}";
                    $this->dbLink->query($sql);
                }
                if($username != $u['userName']){
                    $sql = "UPDATE x2_user SET userName = '$username' WHERE id = {$u['id']}";
                    $this->dbLink->query($sql);
                }
                if($uid != $u['uid']){
                    $sql = "UPDATE x2_user SET uid = '$uid' WHERE id = {$u['id']}";
                    $this->dbLink->query($sql);
                }
                $sql = "select * from x2_user WHERE  id=$userId";
                $u = $this->dbLink->fetch_first($sql);
                $data = $u;
            }
        }else{
