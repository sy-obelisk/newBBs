...
function uc_user_register($username, $password, $email, $phone,$source,$createTime, $questionid = '', $answer = '', $regip = '') {
	return call_user_func(UC_API_FUNC, 'user', 'register', array('username'=>$username, 'password'=>$password, 'email'=>$email,'phone'=>$phone,'source' => $source,'regdate' => $createTime, 'questionid'=>$questionid, 'answer'=>$answer, 'regip' => $regip));
}
...
function uc_user_edit_integral($username,$behavior,$type,$integral) {
...
function uc_user_edit($username, $oldpw, $newpw, $email, $phone,$ignoreoldpw = 0, $questionid = '', $answer = '') {
	return call_user_func(UC_API_FUNC, 'user', 'edit', array('username'=>$username, 'oldpw'=>$oldpw, 'newpw'=>$newpw, 'email'=>$email, 'phone'=>$phone,'ignoreoldpw'=>$ignoreoldpw, 'questionid'=>$questionid, 'answer'=>$answer));
}
